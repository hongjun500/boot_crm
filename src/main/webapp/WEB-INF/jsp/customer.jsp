...
<div id="wrapper">
	...
	<nav class="navbar navbar-default navbar-static-top" role="navigation"
		...
		<div class="navbar-header">
			<a class="navbar-brand" href="<%=basePath%>/list.action">BOOT客户管理系统 v2.0</a>
		</div>
		<!-- 导航栏右侧图标部分 -->
					...
					<div class="form-group">
						...
						<select	class="form-control" id="custLevel" name="custLevel">
							...
							<c:forEach items="${levelType}" var="item">
								<option value="${item.dict_id}"
                                        <c:if test="${item.dict_id == custLevel}"> </c:if>>
										${item.dict_item_name}
								</option>
							</c:forEach>
...
<script type="text/javascript">
    ...
    function createCustomer() {
        ...
        var new_custName=document.getElementById(new_customerName);
        console.log(new_custName)
    }
    // 通过id获取修改的客户信息
   ...
   function  deleteCustomer(id) {
       if(confirm("确定要删除吗")){
           $.post("<%=basePath%>delete.action",{id:id},
			   function (data) {
                   if(data =="OK"){
                       alert("客户删除成功！");
                       window.location.reload();
